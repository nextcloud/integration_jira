OC.L10N.register(
    "integration_jira",
    {
    "Error getting OAuth accessible resource list." : "OAuth erişilebilir kaynak listesi alınırken sorun çıktı.",
    "Error getting OAuth access token." : "OAuth erişim kodu alınırken sorun çıktı.",
    "Error during OAuth exchanges" : "OAuth alışverişinde sorun çıktı",
    "Jira notifications" : "Jira bildirimleri",
    "Jira filtered notifications" : "Jira tarafından süzülmüş bildirimler",
    "Jira" : "Jira",
    "_You have %n open issue with recent activity in Jira._::_You have %n open issues with recent activity in Jira._" : ["Jira üzerinde son etkinlikle ilgili %n açık sorununuz var.","Jira üzerinde son etkinlikle ilgili %n açık sorununuz var."],
    "Jira issues" : "Jira sorunları",
    "Bad HTTP method" : "HTTP yöntemi hatalı",
    "Bad credentials" : "Kimlik doğrulama bilgileri hatalı",
    "Connected accounts" : "Bağlı hesaplar",
    "Jira integration" : "Jira bütünleştirmesi",
    "Integration of Jira user support/ticketing solution" : "Jira kullanıcı destek kaydı açma ve destek alma çözümü",
    "Jira integration provides a dashboard widget displaying your important notifications,\na unified search provider to search for issues\nand notifications about recent activity related to your assigned issues." : "Jira bütünleştirmesi önemli bildirimleri, arama özelliği ve bildirilen sorunlar ile ilgili son işlemleri\ngörüntüleyen bir pano bileşeni sağlar.",
    "Jira admin options saved" : "Jira yönetici ayarları kaydedildi",
    "Failed to save Jira admin options" : "Jira yönetici ayarları kaydedilemedi",
    "If you want to allow your Nextcloud users to use OAuth to authenticate to Jira, create an application in your Jira admin settings and set the ID and secret here." : "Nextcloud kullanıcılarınızın Jira kimlik doğrulaması için OAuth uygulamasını kullanmasını istiyorsanız, Jira yönetici ayarlarınızdan bir uygulama oluşturun ve aşağıya kimlik ve parola bilgilerini yazın.",
    "Jira app settings" : "Jira uygulama ayarları",
    "Make sure you set the redirection/callback URL to" : "Yönlendirme/geri çağırma adresini şuna ayarladığınızdan emin olun",
    "Don't forget to make your Jira OAuth application public." : "Jira OAuth uygulamasını herkese açık olarak yayınlamaya unutmayın.",
    "How to make Jira OAuth public" : "Jira OAuth nasıl herkese açık yapılır",
    "Put the \"Client ID\" and \"Client secret\" below. Your Nextcloud users will then see a \"Connect to Jira\" button in their personal settings." : "\"İstemci kimliği\" ve \"İstemci sırrı\" bilgilerini aşağıya yazın. Bundan sonra Nextcloud kullanıcılarınız kişisel ayarlarında \"Jira bağlantısı kur\" seçeneğini görecek.",
    "Client ID" : "İstemci kimliği",
    "ID of your application" : "Uygulamanızın kimliği",
    "Client secret" : "İstemci sırrı",
    "Your application secret" : "Uygulama sırrınız",
    "Restrict self hosted URL to" : "Kişisel barındırma adresi şununla kısıtlansın",
    "Instance address" : "Kopya adresi",
    "Enable link previews" : "Bağlantılar ön izlenebilsin",
    "Unassigned" : "Atanmamış",
    "Type" : "Tür",
    "Status" : "Durum",
    "Priority" : "Öncelik",
    "Labels" : "Etiketler",
    "Assignee" : "Atanan",
    "Created" : "Oluşturulma",
    "Updated" : "Güncellenme",
    "Successfully connected to Jira!" : "Jira ile bağlantı kuruldu!",
    "OAuth access token could not be obtained:" : "OAuth erişim kodu alınamadı:",
    "Jira options saved" : "Jira ayarları kaydedildi",
    "Failed to save Jira options" : "Jira ayarları kaydedilemedi",
    "Failed to get Jira projects" : "Jira projeleri alınamadı",
    "Impossible to connect to Jira instance" : "Jira kopyası ile bağlantı kurulamadı",
    "Login/password are invalid or account is locked" : "Kullanıcı adı ya da parola geçersiz ya da hesap kilitli",
    "Failed to connect to Jira Software" : "Jira Software bağlantısı kurulamadı",
    "Failed to save Jira OAuth state" : "Jira OAuth durumu kaydedilemedi",
    "Connected as {username}" : "{username} olarak bağlantı kuruldu",
    "Disconnect from Jira" : "Jira bağlantısını kes",
    "Enable unified search for tickets" : "Destek kayıtları için birleşik arama yapılabilsin",
    "Enable user link preview" : "Kullanıcı bağlantıları ön izlenebilsin",
    "Enable notifications for open tickets" : "Açık destek kayıtları için bildirimler kullanılsın",
    "Warning, everything you type in the search bar will be sent to Jira." : "Uyarı, arama çubuğuna yazdığınız her şey Jira üzerine gönderilecek.",
    "Select Jira projects for the Dashboard widget" : "Pano bileşeni için Jira projelerini seçin",
    "Select Jira projects" : "Jira projelerini seçin",
    "Only projects available to your Jira account are listed." : "Yalnızca Jira hesabınız ile kullanılabilecek projeler listeleniyor.",
    "Jira Cloud" : "Jira Cloud",
    "Connect to Jira Cloud" : "Jira Cloud bağlantısı kur",
    "Ask your Nextcloud administrator to configure a Jira Cloud OAuth app in order to be able to connect to this service." : "Bu hizmete bağlanabilmek için Nextcloud yöneticinizden bir Jira Cloud OAuth uygulaması yapılandırmasını isteyin.",
    "Self-hosted Jira Software" : "Kişisel barındırılan Jira Software",
    "Jira self-hosted instance address" : "Kişisel barındırılan Jira kopyasının adresi",
    "Jira address" : "Jira adresi",
    "User" : "Kullanıcı adı",
    "Jira user name" : "Jira kullanıcı adı",
    "Password" : "Parola",
    "Jira password" : "Jira parolası",
    "Connect to this Jira instance" : "Bu Jira kopyası ile bağlantı kur",
    "No Jira account connected" : "Bağlı bir Jira hesabı yok",
    "Error connecting to Jira" : "Jira ile bağlantı kurulurken sorun çıktı",
    "No Jira notifications!" : "Herhangi bir Jira bildirimi yok!",
    "Failed to get Jira notifications" : "Jira bildirimleri alınamadı",
    "Connect to Jira" : "Jira bağlantısı kur"
},
"nplurals=2; plural=(n > 1);");
